...
// GET /api/analyze/history — Retrieve saved analyses
router.get('/history', async (req: Request, res: Response) => {
  try {
    const analyses = await AnalysisModel.find().sort({ createdAt: -1 }).limit(50);
...
// DELETE /api/analyze/history/:id — Delete a specific analysis record
...
// DELETE /api/analyze/history — Clear all history records
router.delete('/history', async (req: Request, res: Response) => {
  try {
    await AnalysisModel.deleteMany({});
    console.log('[MongoDB] Cleared all history records ✓');
